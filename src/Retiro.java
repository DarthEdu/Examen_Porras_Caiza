        ...
        ENTERButton.addActionListener (new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if(retiro > saldoactual){
                    JOptionPane.showMessageDialog(null, "No tiene saldo suficiente ", "Voucher", JOptionPane.WARNING_MESSAGE);
                    textField1.setText("");
                }
                else {
                    Saldo.saldobanco = saldoactual - retiro;
                    textField1.setText("");
                    int respuesta = JOptionPane.showConfirmDialog(null, "Desea ver el voucher?",
                            "Imprimir", JOptionPane.YES_NO_OPTION);
                    try {
                        if (respuesta == JOptionPane.YES_OPTION) {
                            JOptionPane.showMessageDialog(null, "Su saldo anterior era: " + saldoactual +
                                    "\nSu saldo actual es " + Saldo.saldobanco + "\nSe realizó un retiro de: " + retiro +
                                    "\nGracias por usar nuestro servicio", "Voucher", JOptionPane.WARNING_MESSAGE);
                        }
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
